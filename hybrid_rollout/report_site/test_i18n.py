...
class ReportLanguageTests(unittest.TestCase):
    ...
    def test_public_rollout_records_link_is_bilingual_and_next_to_repository(self):
        publication=json.loads((CONTENT/'publication.json').read_text())
        self.assertEqual(publication['dataset'],'https://huggingface.co/datasets/YuMoool/astra-robodojo-rollouts')
        self.assertEqual(self.locale['ui']['评测记录'],'Rollout Records')
        self.assertNotIn('数据集（Hugging Face）',self.locale['ui'])
        source=(CONTENT/'ReportContent.jsx').read_text()
        self.assertIn("{t('评测记录')}",source)
        self.assertIn('className="rr-dataset-link" href={publication.dataset} target="_blank" rel="noopener noreferrer"',source)
        self.assertLess(source.index('className="rr-repository-link"'),source.index('className="rr-dataset-link"'))
        self.assertLess(source.index('className="rr-dataset-link"'),source.index("{t('RoboDojo 视频库')}"))
