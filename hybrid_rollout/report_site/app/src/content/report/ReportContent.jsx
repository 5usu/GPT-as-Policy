...
function ReportArticle() {
  ...
  return <article className="rr-report" lang={language==='zh'?'zh-CN':'en'}>
    <header className="rr-hero">
      ...
      <div className="rr-project-links"><a className="rr-repository-link" href={publication.repository} target="_blank" rel="noopener noreferrer" aria-label={t('在 GitHub 查看代码')}>GitHub</a><a className="rr-dataset-link" href={publication.dataset} target="_blank" rel="noopener noreferrer">{t('评测记录')}</a>{galleryAvailable&&<a href={`gallery.html?lang=${language}`}>{t('RoboDojo 视频库')}</a>}{robolabGalleryAvailable&&<a href={`robolab-gallery.html?lang=${language}`}>{t('RoboLab 视频库')}</a>}<a href="scores.csv">{t('评测数据')}</a><a href="#references">{t('参考文献')}</a></div>
    </header>
